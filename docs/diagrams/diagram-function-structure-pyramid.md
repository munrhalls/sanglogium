...
graph TD
    ...
    A([Single Responsibility]):::state --> B([Small Functions]):::action
    B --> C([Descriptive Names]):::action
    C --> D([Few Parameters]):::logic
    D --> E([One Abstraction Level]):::logic
    E --> F([Clean Codebase]):::state
```

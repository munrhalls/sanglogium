...
graph TD
    ...
    A([Ubiquitous Language]):::state --> B([Bounded Context A]):::state
    A --> C([Bounded Context B]):::state
    B --> D{Context Mapping}:::logic
    C --> D
    D --> E([Partnership]):::action
    D --> F([Customer-Supplier]):::action
    D --> G([Anti-Corruption Layer]):::action
```

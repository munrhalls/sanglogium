...
- `inlineCss: true` (native, stream-aware CSS inlining — do not re-enable `optimizeCss`/critters: it post-processes the fully-rendered HTML for critical-CSS extraction, which forces Next to buffer the whole SSR/RSC response and defeats any per-row `Suspense` streaming. Confirmed via HAR in an earlier streamed-grid implementation: with it on, all product images fired within a 2ms window instead of streaming in per row.)

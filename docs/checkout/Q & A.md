...
# Q & A - CRITICAL Q'S ONLY INTELLIGENCE GATHERING - HAPPY PATH TRACER ONLY, GUEST CHECKOUT ONLY (AUTH OUT OF SCOPE)
...
**How:** Your Next.js server holds a small, static configuration file matching the Furgonetka pricing brackets (e.g., InPost Gabaryt A = 16.99 PLN).
...
### The Advanced Alternative: The "Mini-Estimator" Component

If your business requirement dictates that the user must know the exact DE/GB price before clicking checkout, you use the Client-Side Mini-Estimator pattern.

**The UI:** Below the basket total, you render a small Client Component:
`[Dropdown: Country] [Input: Postal Code] [Button: Calculate]`

**The Action:** When the user clicks "Calculate", it triggers a Next.js Server Action.

**The Server Action:**

1. Takes the user's input `{ country: "DE", postalCode: "10115" }`.
2. Reads the parcelData already fetched by Sanity.
3. Now it makes the API call to Packlink.
4. Returns the exact rates to the client component.
...
By structuring your work this way, you can generate one `/sprint` `.todo` file for Sprint 1, feed those 3-4 scopes into Beads, and have SWE 1.6 build it. Once Sprint 1 passes the build gate, you move to Sprint 2.
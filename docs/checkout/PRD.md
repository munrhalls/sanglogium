# Scope: basket checkout button → address page (Google API verification) → shipping page (Shippo API options) → payment page (Stripe Elements) → order creation (stock update from reservedStock) → reservation cleanup → success page

## Definition of Done - happy path

✔ [ ] Address page validates user input, calls Google API for address verification, saves verified address to reservation document, redirects to shipping page

...
## PHASE 4: Output Sprint File ([SPRINT_NAME].todo)

### Mandatory Opus Specification Quality Requirements:

1. **Gap Coverage Mapping:** Every Scope Contract MUST explicitly reference audit gaps (G1, G2, G3...).
2. **Line-Number Precision:** Include exact file paths with line numbers/ranges and class names for before/after states.
3. **Constraint-First Architecture:** Include a "Scope Lock Rules" section FIRST (what NOT to touch), including Anti-patterns.
4. **Layer-Based Sequencing:** Strictly follow Pass 1 (Skeleton), Pass 2 (Data), Pass 3 (Build: Structure → Layout → Surface → Interaction).
5. **Verifiable DoDs:** Include build gates after EVERY Scope Contract with objective pass/fail criteria.
6. **Functional Grouping:** Group contracts into:
   - Group A: Foundation (layout, structure)
   - Group B: Core UX (product display)
   - Group C: Functional systems (filters)
   - Group D: Polish (skeletons, semantics)

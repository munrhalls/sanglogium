// Simple setup script that loads .env and creates test data
const fs = require('fs')
...
async function setupTestData() {
  ...
  try {
    ...
    // Give Sanity a moment to index
    await new Promise(resolve => setTimeout(resolve, 1000))

    const verifyProducts = await backendClient.fetch(
      `*[_type == "product" && slug.current in ["test-product-alpha", "test-product-beta", "test-product-gamma"]] {_id, name, slug}`
    )

    console.log(`  Found ${verifyProducts.length} test products`)
...
module.exports = { setupTestData, verifyStripePrices, cleanupTestData }

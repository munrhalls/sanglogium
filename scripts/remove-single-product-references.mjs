...
/**
 * Script to remove references for a single product
 * Usage: node scripts/remove-single-product-references.mjs <productId> [--dry-run]
 */
...
if (!productId) {
  console.error('Usage: node scripts/remove-single-product-references.mjs <productId> [--dry-run]');
  process.exit(1);
}
...
async function main() {
  console.log('========================================');
  console.log('Remove Single Product References from CMS');
  console.log('========================================\n');
  ...
  if (!dryRun) {
    console.log('⚠️  WARNING: This will remove all references to this product from CMS');
  ...
  if (referencingDocs.length === 0) {
    console.log('✅ No references found. Product can be deleted directly.');
    process.exit(0);
  }
  ...
  if (dryRun) {
    ...
    console.log(`\nYou can now delete product ${productId}`);
  }
}

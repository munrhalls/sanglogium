...
export function Hero() {
  return <div className="border-2 border-red-500">Hero</div>
}

// ProductGrid.tsx
export function ProductGrid() {
  return <div className="border-2 border-blue-500">ProductGrid</div>
}
```
...
**Example prompt:**
```
CONTEXT: Empty ProductCard.tsx in Next.js 15 project.
TARGET: Generate JSX skeleton for a product card.
...
**Example prompt:**
```
CONTEXT: ProductCard.tsx has this structure: [paste JSX].
TARGET: Add Tailwind layout classes for responsive card layout.
...
For a page with 3 components (Hero, ProductGrid, Footer):

**Pass 1 — Skeleton (all components):**
- Hero: Text only + border
- ProductGrid: Text only + border
- Footer: Text only + border
- Lock: Page scrolls, all present

**Pass 2 — Data (all components):**
- Hero: Real hero data, no styling
- ProductGrid: Real products, no styling
- Footer: Real links, no styling
...
**Mixing Passes:**
- Building Hero fully (Pass 3) while ProductGrid has no data (Pass 2 not done)
...
## Files That Must Reference This Pattern

- `.windsurf/workflows/sprint.md` — scope contracts use this sequencing
- `.windsurf/workflows/audit.md` — RWD section maps to this pattern
- Any sprint `.todo` file — DoDs follow Pass/Layer structure
- Component source files — comments may reference Pass/Layer for context
...
Curriculum source: ai-webdev-spatial-curriculum-v3.md
Pattern validated: PLP Design System Alignment sprint (10 scope contracts, build passing)

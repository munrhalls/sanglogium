...
- React 18 StrictMode + `react-stripe-js` **has known interaction bugs** around `Elements` re-mounting. Thes canonical fix is well-understood: `loadStripe` outside render (module-scope singleton) + `options` object stabilised with `useMemo` + conditionally rendering `<Elements>` only when `clientSecret` is defined.

---
product_id: GPjMdcfFWZVrKyR2PB4QYo
product_slug: topping-dx3-pro-bluetooth-dac-amp
brand: Topping
name: TOPPING DX3 Pro+ Bluetooth DAC/Amp
...
- **deviceConnectivity** (marketing-fact): `wired-wireless` — HIFiGO lists 'Hi-Res Bluetooth Transmission' and 'Multiple Inputs (Coax, USB, Optical, Bluetooth)'.
...
- **inputs** (hard-spec): `['usb', 'optical', 'coaxial', 'bluetooth']` — HIFiGO 'Technical Parameters': 'Multiple Inputs(Coax, USB, Optical, Bluetooth)' and 'USB decoding parameters: PCM 32-Bit/768kHz, DSD512 natively'
...
- DX3 Pro+ is a DAC/headphone-amp combo. `deviceType` is recorded as `dac` (the only applicable should-be value), but `outputs`, `trigger12v`, and `remoteControlIncluded` are domain-gated to amplifier `deviceType` values and remain `null` despite the source mentioning a remote and trigger.

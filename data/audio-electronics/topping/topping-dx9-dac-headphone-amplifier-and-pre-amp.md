---
product_id: MrEMtYwMtrFDGWmRnN9RG0
...
- **deviceConnectivity** (marketing-fact): `wired-wireless` — HIFiGO features list 'Bluetooth V5.1 Connectivity For Signal Input' and 'Comprehensive List of Inputs and Outputs'.
...
- **maxSampleRateBitDepth**: `32-bit / 768 kHz` — HIFiGO: 'supports advanced high-resolution PCM and DSD audio signals' and store highlights 'up to PCM 32bit/768kHz'; **dsdSupport**: `dsd256-plus` — store listing and web sources list 'DSD512' support; **hiResCertification**: `['hi-res-audio']` — store product highlights: 'exceptional Hi-Res audio experience'; **dacChipsetFamily**: `null`; **streamingPlatformSupport**: `null`; **networkConnection**: `null`
...
- The issue lists two DX9 product records with the same name and price. Both source to the same HIFiGO/store DX9 Discrete product; the two files are therefore identical in spec data. The DX9 line also has a separate AK4499EQ variant on the TOPPING store, but the issue's two records cannot be mapped to the two store variants from the available data.
- The DAC architecture (PSRM, 1-bit discrete) is not in the `dacChipsetFamily` enum, so `null` is recorded despite the source being clear.

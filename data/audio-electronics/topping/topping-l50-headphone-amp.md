---
product_id: DZc43yHr6ydfgE7zB42o6O
product_slug: topping-l50-headphone-amp
brand: Topping
name: TOPPING L50 Headphone Amp
...
- https://apos.audio/products/topping-l50-nfca-headphone-amplifier
...
- **deviceType** (marketing-fact): `null` — official store titles the product 'TOPPING L50 NFCA Headphone Amplifier'. The `should-be-audio-electronics.md` `deviceType` enum does not include `headphone-amp`, so the value is `null` rather than force-fit.
- **deviceConnectivity** (marketing-fact): `wired` — wired line input; no Bluetooth or Wi-Fi listed; the unit operates with wired line/digital inputs.
...
- HIFiGO lists L50 variant titles including 'Blue' and 'Red', while the TOPPING official store only lists 'Black' and 'Silver'. The manufacturer store (higher-tier source) is used for `finishColor`.

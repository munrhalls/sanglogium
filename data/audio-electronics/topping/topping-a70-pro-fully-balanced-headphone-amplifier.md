---
product_id: DZc43yHr6ydfgE7zB42mdu
product_slug: topping-a70-pro-fully-balanced-headphone-amplifier
brand: Topping
name: TOPPING A70 Pro Fully Balanced Headphone Amplifier
...
- **deviceType** (marketing-fact): `null` — HIFiGO lists the product as 'TOPPING A70Pro Amplifier Fully Balanced R2R Volume Control Module Headphone Amp'. The `should-be-audio-electronics.md` `deviceType` enum does not include `headphone-amp`, so the value is `null` rather than force-fit.
- **deviceConnectivity** (marketing-fact): `wired` — no Bluetooth or Wi-Fi input is listed; connections are XLR/RCA line inputs and 4.4mm/6.35mm/XLR headphone outputs; the unit operates with wired line/digital inputs.
...
- Source pages state the A70 Pro includes a 12V trigger and remote control, but `trigger12v` and `remoteControlIncluded` are domain-gated to amplifier `deviceType` values and remain `null` because `deviceType` is `null`.

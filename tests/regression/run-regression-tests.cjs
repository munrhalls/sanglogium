 ...
 *
 * Run: node tests/regression/run-regression-tests.cjs
 */
...
function runVfsTests() {
  ...
  test("VFS critical leaf slugs exist", () => {
    const data = JSON.parse(readFile("data/catalogue-index.json"));
    // Leaf slugs that should exist (headers like "headphones" are not directly navigable)
    const criticalLeafSlugs = [
      "open-back",
      "closed-back",
      "desktop-amps",
      "portable-amps",
    ...
    const missing = criticalLeafSlugs.filter((slug) => !data.slugToIdMap[slug]);

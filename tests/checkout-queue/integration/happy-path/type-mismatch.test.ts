...
describe('Checkout queue — type-mismatch rejection', () => {
  ...
  it('rejects payload missing stripePriceId / price_data / createdAt with 400 and no processing trace', async () => {
    const invalid = {
      basketReservation: [{ _id: 'test-product-1', quantity: 1 }],
      // missing createdAt, missing stripePriceId and price_data inside item
    }

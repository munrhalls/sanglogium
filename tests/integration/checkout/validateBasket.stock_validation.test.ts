/**
 * Integration tests for validateBasket server action - Stock validation step
 * Tests stock shortage scenarios and inventory validation
 */
...
describe('validateBasket - Stock validation', () => {
  ...
  it('should return FAIL_VALIDATION with INVENTORY payload on stock shortage', async () => {
    // Mock Sanity response with insufficient stock on item-1
    mockSanityFetch.mockResolvedValue([
      { _id: 'item-1', name: 'Product 1', displayPrice: 100, stock: 1, stripePriceId: 'price_1' }, // Only 1 in stock, basket wants 2
  ...
  it('should handle undefined stock field gracefully', async () => {
    // Mock Sanity response with undefined stock (should not trigger inventory check)
    mockSanityFetch.mockResolvedValue([
      { _id: 'item-1', name: 'Product 1', displayPrice: 100, stock: undefined, stripePriceId: 'price_1' },
      { _id: 'item-2', name: 'Product 2', displayPrice: 100, stock: undefined, stripePriceId: 'price_2' }
    ]);

    const result = await validateBasket(mockBasketPayload, 'test-key');

    // Should not fail validation (proceeds to next steps)
    expect(result.outcome).not.toBe("FAIL_VALIDATION");
    expect(mockSanityFetch).toHaveBeenCalledTimes(1);
  });

  it('should proceed when stock is sufficient', async () => {
    // Mock Sanity response with sufficient stock
    mockSanityFetch.mockResolvedValue([
    ...
    // Should pass stock validation and proceed to next steps
    expect(result.outcome).not.toBe("FAIL_VALIDATION");
    expect(mockSanityFetch).toHaveBeenCalledTimes(1);

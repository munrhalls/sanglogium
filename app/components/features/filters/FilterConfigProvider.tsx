...
interface FilterConfigProviderProps {
  categoryKeys: string[];
  children: (props: { filters: FilterGroup[]; priceRange: { minPrice: number | null; maxPrice: number | null }; maxStock: number | null }) => React.ReactNode;
}
...
export async function FilterConfigProvider({ categoryKeys, children }: FilterConfigProviderProps) {
  ...
  return <>{children({ filters: filterResult.filters, priceRange: filterResult.priceRange, maxStock: filterResult.maxStock })}</>;
}

...
export default function IemCard({ product, idx }: { product: IemProduct; idx: number }) {
    ...
    return (
        <a href={`/products/${product.slug}`} className="block group">
            ...
            <div className="relative flex justify-center items-center aspect-square w-full overflow-hidden rounded-none bg-surface-productImage pt-8 xs:pt-8 md:pt-12 pb-2 xs:pb-4 cursor-pointer">
                <Image
                ...
                />
                <div className="absolute left-2 top-1 xs:top-4">
                    <span className="text-[7px] xs:text-small font-bold uppercase tracking-editorial text-brand-900 whitespace-nowrap xs:whitespace-normal">{product.brand}</span>
                </div>
            </div>

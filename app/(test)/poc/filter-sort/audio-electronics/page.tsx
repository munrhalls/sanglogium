...
import type { AudioElectronicsProduct } from './lib/types';
import rawDataset from './dataset.json';

export const dynamic = 'force-dynamic';

export const metadata: Metadata = {
  title: 'Filter & Sort POC — Audio Electronics',
  robots: { index: false, follow: false },
};

const DATASET = rawDataset as unknown as AudioElectronicsProduct[];
 ...
 */
function fetchChunk(products: AudioElectronicsProduct[], offset: number, limit: number, chunkIndex: number): Promise<AudioElectronicsProduct[]> {
...
export default async function FilterSortAudioElectronicsPocPage({ searchParams }: PocPageProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-catalogue px-4 pb-12 md:px-8">
      <div className="pt-8">
        <ShopHeader title="Audio Electronics" overline="Filter & Sort Proof of Concept" />
      </div>
      ...
      {sorted.length === 0 ? (
        <div className="flex flex-col items-center gap-2 rounded-md border border-border-secondary bg-surface-elevated py-16 text-center" data-testid="poc-empty-results">
          <p className="type-body text-text-primary">No audio electronics match these filters.</p>

...
// snapshot (see scripts/poc-fetch-headphones-dataset.mjs), so filtering here
...
function productValues(product: AudioElectronicsProduct, facet: FacetDef): string[] {
...
export function productMatchesState(
  product: AudioElectronicsProduct,
...
export function filterProducts(products: AudioElectronicsProduct[], state: FilterSortState): AudioElectronicsProduct[] {
...
function weightedRating(p: AudioElectronicsProduct, meanRating: number, minVotes = 15): number {
  const v = p.ratingCount;
  return (v / (v + minVotes)) * p.rating + (minVotes / (v + minVotes)) * meanRating;
}

export function sortProducts(products: AudioElectronicsProduct[], sort: SortValue): AudioElectronicsProduct[] {
...
export function computeFacetCounts(
  products: AudioElectronicsProduct[],
  state: FilterSortState,
...
export function computeBooleanCounts(products: AudioElectronicsProduct[], state: FilterSortState): Record<string, number> {
...
export function derivedBrandLabels(products: AudioElectronicsProduct[]): Record<string, string> {

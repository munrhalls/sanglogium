    ...
    <div className="max-w-2xl mx-auto w-full px-4 md:px-0 pt-10 pb-28 md:pb-16">
      ...
      <div className="flex md:hidden items-center justify-center gap-2 mb-8">
        {[1, 2, 3].map((i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            {i < 3 && <div className="w-4 h-px bg-surface-elevated" />}
          </div>
        ))}
      </div>
      ...
      <div className="space-y-3">
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className="flex items-center justify-between gap-4 rounded-lg border border-border-secondary px-4 py-4 md:py-5 bg-surface-card"
          >
            <div className="flex items-center gap-4 flex-1">
              <div className="w-5 h-5 rounded-full bg-surface-elevated animate-pulse flex-shrink-0" />

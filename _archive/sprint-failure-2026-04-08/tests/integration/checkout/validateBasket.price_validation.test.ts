...
describe('validateBasket - Price validation', () => {
  ...
  it('should return FAIL_VALIDATION with PRICE payload on price mismatch', async () => {
    ...
    // Calculate expected total: (100 * 2) + (120 * 1) = 320
    // Basket total is 300, so there's a mismatch
      ...
      discrepancy: {
        ...
        items: [
          ...
          },
          {
            ...
            expected: 120, // Sanity price
            actual: 100   // Basket price per item for this product
          }
        ]
      }
    });
  ...
  it('should proceed when prices match', async () => {
    ...
    // For basket total of 300 with 2x item-1 and 1x item-2:
    // If item-1 is 100, then item-2 should be 100 to match: (100 * 2) + (100 * 1) = 300
    mockSanityFetch.mockResolvedValue([
      { _id: 'item-1', name: 'Product 1', displayPrice: 100, stock: 10, stripePriceId: 'price_1' },
